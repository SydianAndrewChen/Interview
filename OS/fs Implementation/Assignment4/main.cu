...
#define SUPERBLOCK_SIZE 4096 //4KB 4096 will control 4096*8 = 32768
...
// volume (disk storage)
__device__ __managed__ uchar volume[VOLUME_SIZE]{0};

__device__ __managed__ uchar temp[TEMP_SIZE];
...
__global__ void mykernel(uchar *input, uchar *output) {
  ...
  fs_init(&fs, volume, SUPERBLOCK_SIZE, FCB_SIZE, FCB_ENTRIES, 
			...
			MAX_FILE_NUM, MAX_FILE_SIZE, FILE_BASE_ADDRESS, temp);
...
int main() {
	cudaDeviceSetLimit(cudaLimitStackSize, 1024 * 1024 * 1024);
